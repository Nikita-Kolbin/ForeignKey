...
type CompletedOrdersGetter interface {
	GetCompletedOrders(websiteId int) ([]storage.Order, error)
	GetWebsite(alias string) (websiteId, adminId int, err error)
}

// NewGetCompletedOrders godoc
...
// @Router /order/get-completed/{alias} [get]
func NewGetCompletedOrders(cog CompletedOrdersGetter, log *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		websiteId, adminId, err := cog.GetWebsite(alias)
		if err != nil {
		...
		orders, err := cog.GetCompletedOrders(websiteId)
		if err != nil {
			log.Error("failed to get website", slog.String("err", err.Error()))
